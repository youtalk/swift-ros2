...
Swift 5.9+ on Apple platforms; Swift 6.2+ on Linux (required for `.artifactbundle` `staticLibrary` support); Windows currently requires Swift 6.3.1. CI runs `macos-15` (Apple Silicon, Xcode 16.2), a Swift 6.2 Linux matrix (Ubuntu 22.04 / 24.04 on x86_64, Ubuntu 22.04 on aarch64), and Swift 6.3.1 on Windows (`windows-latest`).
