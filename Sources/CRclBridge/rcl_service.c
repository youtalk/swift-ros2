...
// concurrent access to rcl entities (rcl_send_response is the documented
...
int crcl_service_send_response(
    ...
    if (!s || !request_id || (!buf && len > 0)) {
        crcl__set_error("crcl_service_send_response: NULL service/request_id/buffer");
        return -1;
    }
